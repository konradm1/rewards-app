...
class RewardReportServiceTest {
    ...
    void prepareReportForCustomerWhenDataExists() {
        ...
        when(purchaseService.providePurchaseRecordsForCustomer(1L, Currency.getInstance(Locale.US), today.withDayOfMonth(1).withHour(0).withMinute(0).withSecond(0).withNano(0).minusMonths(2)))
    ...
    void prepareReportForCustomerWhenDataNotExists() {
        ...
        when(purchaseService.providePurchaseRecordsForCustomer(1L, Currency.getInstance(Locale.US), LocalDateTime.now().withDayOfMonth(1).withHour(0).withMinute(0).withSecond(0).withNano(0).minusMonths(2)))
                .thenReturn(purchaseDateAndAmountRecords);
